...
class Cdm:
    ...
    def open_session(self, init_data_b64: str, device: DeviceConfig, raw=False, offline=False):
        ...
        if device.session_id_type == "android":
            ...
            rand_ascii: str = "".join(random.choice("ABCDEF0123456789") for _ in range(16))
            ...
            session_id = rand_bytes.hex()
        else:
            # other formats NYI
            raise InternalServerError("device type is unusable")
        if raw:
            # used for NF key exchange, where they don't provide a valid PSSH
            init_data = base64.b64decode(init_data_b64)
            self.raw_pssh = True
        else:
        ...
        if init_data:
            new_session = Session(session_id, init_data, device, raw, offline)
        else:
            raise BadRequest("unable to parse init data")
        self.sessions[session_id] = new_session
        self.logger.info("session opened and init data parsed successfully")
        return session_id

    def _parse_init_data(self, init_data_b64: str) -> bytes:
        parsed_init_data = wv_proto2.WidevineCencHeader()
        try:
            self.logger.debug("trying to parse init_data directly")
            init_data = base64.b64decode(init_data_b64)
            if init_data[8] == 0:
                parsed_init_data.ParseFromString(init_data[32:])
            else:
                kid_count = int.from_bytes(init_data[0x1C:0x20], byteorder="big")
                parsed_init_data.ParseFromString(init_data[36 + 16 * kid_count :])
        except DecodeError:
    ...
    def set_service_certificate(self, session_id: str, cert_b64: str):
        ...
        if message.Type:
            ...
            try:
                ...
                raise BadRequest("failed to parse service certificate")
    ...
    def get_license_request(self, session_id: str) -> str:
        ...
        if not os.path.exists(session.device_config.device_client_id_blob_filename):
            raise InternalServerError("no client ID blob available for this device")

        with open(session.device_config.device_client_id_blob_filename, "rb") as f:
            try:
                cid_bytes = client_id.ParseFromString(f.read())
            except DecodeError:
                raise DecodeError("client id failed to parse as protobuf")
        ...
        license_request.Msg.ContentId.CencId.RequestId = session_id.encode()
            ...
            if session.device_config.vmp:
                ...
                with open(session.device_config.device_vmp_blob_filename, "rb") as f:
                    try:
                        ...
                        raise DecodeError("vmp hashes failed to parse as protobuf")
        ...
        if session.device_config.private_key_available:
            ...
            raise InternalServerError("no device key available")
        ...
        return base64.b64encode(license_request.SerializeToString()).decode()

    def provide_license(self, session_id: str, license_b64: str):
        ...
        if not session.license_request:
            raise BadRequest("generate a license request first!")

        license = wv_proto2.SignedLicense()
        try:
            license.ParseFromString(base64.b64decode(license_b64))
        except DecodeError:
            raise BadRequest("unable to parse license - check protobufs")
        ...
        if lic_hmac.digest() != license.Signature:
            raise BadRequest("license signature doesn't match - exiting...")
    ...
    def get_keys(self, session_id) -> list[Key]:
        if not session_id in self.sessions:
            raise BadRequest("Invalid Session ID")

        return self.sessions[session_id].keys

    def get_content_keys(self, session_id) -> list[Key]:
        if not session_id in self.sessions:
            raise BadRequest("Invalid Session ID")

        return list(filter(lambda x: x.type == "CONTENT", self.sessions[session_id].keys))
